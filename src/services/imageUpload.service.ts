...
const uploadAndInspectItem =
  async (
    ...
    try {
      ...
      const itemImage =
        await itemImageService.createItemImage(
          itemId,
      ...
      const aiResult =
        await aiInspectionService.detectDamage(
          filePath
        );

      const inspection =
        await itemInspectionService.createInspection(
          itemId,
          modelId,
          aiResult
        );

      await itemDefectService.createDefects(
        inspection.id,
        aiResult.damages
      );
